...
export default function About() {
  return (
    <section id="sg-about" style={shell}>
      <div data-gs-reveal="1" style={{ display: "flex", alignItems: "baseline", gap: 18, borderTop: `1px solid ${S.rule}`, paddingTop: 18 }}>
        <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", color: S.accentDeep, fontWeight: 700 }}>01</span>
        <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: S.muted }}>About</span>
      </div>

      {/* Pinned while the statement scales/fades in — GSAP ScrollTrigger
          pin+scrub, the structural analog of the reference site's pinned
          "REDEFINING LIMITS" sequence. */}
      <div data-gs-pin-zoom="1" style={{ minHeight: "50vh", display: "flex", alignItems: "center" }}>
        <div>
          <p data-gs-zoom-target="1" style={{
            margin: "clamp(20px,4vw,32px) 0 0", maxWidth: "22ch", fontFamily: MONO, fontWeight: 700, textTransform: "uppercase",
            fontSize: "clamp(1.6rem,3.6vw,2.6rem)", lineHeight: 1.14, letterSpacing: "-.005em", color: S.ink,
          } as React.CSSProperties}>
            Thirteen years in IT. Six turning reporting chaos into{" "}
            <span style={{ fontFamily: ACCENT_SERIF, fontStyle: "italic", fontWeight: 400, textTransform: "none", color: S.accentDeep }}>platforms</span>.
          </p>
      ...
      <div style={{ marginTop: "clamp(36px,5vw,64px)", ...autoGrid(320), gap: "clamp(28px,4vw,64px)" }}>
        <div style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: 20 }}>
          <p data-gs-reveal="1" style={{ margin: 0, fontSize: "clamp(1rem,1.4vw,1.15rem)", lineHeight: 1.68, color: S.muted }}>
          ...
          <p data-gs-reveal="1" style={{ margin: 0, fontSize: "clamp(1rem,1.4vw,1.15rem)", lineHeight: 1.68, color: S.ink }}>
          ...
          <div data-gs-reveal="1" style={{ marginTop: 8, borderTop: `1px solid ${S.rule}`, paddingTop: 20 }}>
            <p style={{ margin: "0 0 14px", fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: S.soft }}>Certified</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {certs.map((c) => (
                <span key={c} style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".1em", color: S.muted, border: `1px solid ${S.rule}`, borderRadius: 99, padding: "8px 14px" }}>{c}</span>
              ))}
            </div>
          </div>

          <div data-gs-reveal="1" style={{ marginTop: 4, ...autoGrid(200), gap: "22px clamp(20px,3vw,40px)", borderTop: `1px solid ${S.rule}`, paddingTop: 20 }}>
            <div>
              <p style={{ margin: "0 0 10px", fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: S.soft }}>Education</p>
              <p style={{ margin: 0, fontSize: ".98rem", lineHeight: 1.5, color: S.ink }}>B.E. Electrical &amp; Electronics Engineering</p>
              <p style={{ margin: "5px 0 0", fontSize: ".92rem", color: S.soft }}>Anna University, Chennai — 2012</p>
            </div>
            <div>
              <p style={{ margin: "0 0 10px", fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: S.soft }}>Mentoring</p>
              <p style={{ margin: 0, fontSize: ".98rem", lineHeight: 1.5, color: S.ink }}>Foundation for Excellence</p>
              <p style={{ margin: "5px 0 0", fontSize: ".92rem", lineHeight: 1.5, color: S.soft }}>Guiding students on career paths, analytics learning, and internship prep.</p>
            </div>
          </div>
        </div>
        ...
        <ul style={{ listStyle: "none", margin: 0, padding: 0, minWidth: 0 }}>
          {principles.map((p) => (
            <li key={p.title} data-gs-reveal-r="1" style={{ borderTop: `1px solid ${S.rule}`, padding: "22px 0" }}>
              <h3 style={{ margin: 0, fontSize: "1.18rem", fontWeight: 700, color: S.ink }}>{p.title}</h3>
              <p style={{ margin: "8px 0 0", fontSize: ".95rem", lineHeight: 1.6, color: S.soft }}>{p.body}</p>
            </li>
          ))}
        </ul>
      </div>
