import { K, SLAB, autoGrid, shell, Scribble } from "@/components/site/kineticStyles";
...
export default function Impact() {
  return (
    <section id="kt-impact" style={{ ...shell, position: "relative", overflow: "hidden" }}>
      <Scribble
        viewBox="0 0 900 200"
        height={140}
        color={K.rule as unknown as string}
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <div data-reveal="1" style={{ display: "flex", alignItems: "baseline", gap: 18, borderTop: `1px solid ${K.rule}`, paddingTop: 18 }}>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, letterSpacing: ".18em", color: K.accentDeep, fontWeight: 700 }}>05</span>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: K.muted }}>Impact</span>
        </div>
        <div style={{ marginTop: "clamp(30px,4vw,48px)", ...autoGrid(230), gap: "clamp(24px,3vw,40px)" }}>
          {metrics.map((m) => (
            <div key={m.body} data-reveal="1" {...(m.parallax ? { "data-parallax": "1" } : {})} style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontFamily: SLAB, fontWeight: 900, fontSize: "clamp(3rem,6.2vw,4.8rem)", lineHeight: .92, letterSpacing: "-.02em", fontVariantNumeric: "tabular-nums", color: K.ink }}>
                <span data-count-to={m.to} data-decimals={m.decimals || undefined}>0</span>
                {m.suffix && <span style={{ color: K.accentDeep, fontSize: ".32em", verticalAlign: "super" }}>{m.suffix}</span>}
              </p>
              <p style={{ margin: "14px 0 0", fontSize: ".95rem", lineHeight: 1.55, color: K.muted, borderTop: `1px solid ${K.rule}`, paddingTop: 14 }}>{m.body}</p>
            </div>
          ))}
        </div>
      </div>
