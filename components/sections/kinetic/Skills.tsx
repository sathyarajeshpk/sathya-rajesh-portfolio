...
export default function Skills() {
  return (
    <section id="kt-skills" style={{ borderTop: `1px solid ${K.rule}`, background: K.bgSoft }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "clamp(64px,9vw,120px) clamp(20px,4vw,56px)" }}>
        <div data-reveal="1" style={{ display: "flex", alignItems: "baseline", gap: 18, borderTop: `1px solid ${K.rule}`, paddingTop: 18 }}>
          <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", color: K.accentDeep, fontWeight: 700 }}>04</span>
          <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: K.muted }}>Capabilities</span>
        </div>
        <h2 data-reveal="1" style={{ fontFamily: SLAB, fontWeight: 900, textTransform: "uppercase", color: K.ink, margin: "clamp(24px,4vw,40px) 0 clamp(32px,4vw,52px)", fontSize: "clamp(1.8rem,3.6vw,2.8rem)" }}>
          The working toolkit.
        </h2>

        <div style={{ ...autoGrid(230), gap: "0 clamp(24px,3vw,48px)" }}>
          {groups.map((g) => (
            <div key={g.title} data-reveal="1" style={{ borderTop: `1px solid ${K.rule}`, padding: "24px 0" }}>
              <h3 style={{ margin: "0 0 14px", fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: K.accentDeep, fontWeight: 700 }}>{g.title}</h3>
              <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 7, fontSize: ".95rem", color: K.muted }}>
