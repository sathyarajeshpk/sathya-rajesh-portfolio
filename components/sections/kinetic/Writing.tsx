...
export default function Writing() {
  ...
  return (
    <section id="kt-writing" style={shell}>
      <div data-reveal="1" style={{ display: "flex", alignItems: "baseline", gap: 18, borderTop: `1px solid ${K.rule}`, paddingTop: 18 }}>
        <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", color: K.accentDeep, fontWeight: 700 }}>07</span>
        <span style={{ fontFamily: MONO, fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: K.muted }}>Writing</span>
      </div>
      <h2 data-reveal="1" style={{ fontFamily: SLAB, fontWeight: 900, textTransform: "uppercase", color: K.ink, margin: "clamp(24px,4vw,40px) 0 clamp(28px,4vw,44px)", fontSize: "clamp(1.8rem,3.6vw,2.8rem)" }}>
        Notes from the work.
      </h2>

      <div data-reveal="1" style={{ borderTop: `1px solid ${K.rule}` }}>
        {posts.map((p, i) => (
          <div key={p.title}>
            <button
              type="button"
              ...
              style={{
                width: "100%", background: "none", border: 0, borderBottom: `1px solid ${K.rule}`,
            ...
            >
              <span style={{ display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>
                <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", color: K.accentDeep }}>{p.meta}</span>
                <span style={{ fontFamily: SLAB, fontWeight: 700, fontSize: "clamp(1.2rem,2.2vw,1.7rem)", lineHeight: 1.2, color: K.ink }}>{p.title}</span>
              </span>
              <span aria-hidden style={{ fontFamily: MONO, fontSize: 18, color: K.accentDeep, flex: "none" }}>{open === i ? "−" : "+"}</span>
            </button>

            {open === i && (
              <div style={{ padding: "22px 0 30px", borderBottom: `1px solid ${K.rule}`, ...autoGrid(280), gap: "20px clamp(24px,4vw,56px)" }}>
                <p style={{ margin: 0, fontSize: "1rem", lineHeight: 1.65, color: K.muted }}>{p.excerpt}</p>
                <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 10, fontSize: ".93rem", lineHeight: 1.55, color: K.soft }}>
                  {p.points.map((pt) => (
                    <li key={pt} style={{ display: "flex", gap: 10 }}>
                      <span style={{ width: 12, height: 1, background: K.accentDeep, flex: "none", marginTop: ".7em" }} />
                      <span>{pt}</span>
                    </li>
                  ))}
                </ul>
